...
- **"Escucha social" fuera del menú**: al entrar expulsaba al login (comprobaba la sesión por Supabase Auth, y el login de la plataforma usa la cookie propia) y, arreglado eso, habría salido vacío: `cm_mentions` tenía **0 registros** desde siempre. Sondeaba una tabla de cuentas vieja con el identificador y el token vacíos y la empresa sin asignar. El código queda en el repositorio; sólo se quitó del menú.
- **Análisis de comentarios, dentro de Comentarios** (lo que prometía Escucha social, por empresa): cada comentario que llega se clasifica al instante por **sentimiento, intención** (pregunta, compra, reclamo, elogio, spam) **y urgencia (0-100)**, y se muestra etiquetado. Arriba, resumen de las últimas 24 h (total, positivos, negativos, cuántos esperan a una persona) y **aviso de crisis** configurable (por defecto: 40% de negativos habiendo al menos 5 comentarios). Nuevo filtro **"Esperan a una persona"**. Y lo más importante: con *No dejar que el agente conteste solo los reclamos y los comentarios negativos* activado, un reclamo **no se responde solo** — queda marcado con el motivo para que lo conteste un asesor. Todo configurable por empresa. Migración **051** aplicada.

### 11 sep 2026
- **Comentarios atendidos a medias**: si la respuesta pública salía y el mensaje al interno fallaba, la ficha quedaba como "Respondido", mostraba el error en rojo y no daba ningún botón para reintentar. Ahora aparecen **Reintentar la respuesta pública** y **Reintentar el mensaje al interno** con lo que falte (el segundo sigue respetando el límite de Meta). El mensaje al interno de Instagram que había fallado se reenvió y entró bien.
- **Comentarios de Instagram, mensaje al interno corregido**: en la prueba (4:50 p. m.) el comentario de Instagram llegó y se respondió en público, pero el privado falló con "(#3) Application does not have the capability". Se enviaba a `/{ig-user-id}/messages` (API de Instagram con inicio de sesión de Instagram); con el token de la página Meta lo rechaza. Ahora sale por `/me/messages`, el mismo camino por el que ya funcionaban los mensajes normales de Instagram. Además, cuando la IA no logra redactar la respuesta pública y se usa el texto fijo, el motivo queda escrito en la ficha del comentario.
- **Comentarios probados en vivo** (Smart Digital, 4:48 p. m.): comentario en un reel de Facebook → llegó por el webhook, se publicó la respuesta pública 23 s después y salió el mensaje al interno, que cayó en el chat que ya existía con ese contacto (no duplicó nada). Corregido de esa prueba: la respuesta pública salía con forma de chat privado (presentación, lista de servicios, 5 frases, 2 emojis) porque el prompt del agente se imponía; ahora la tarea manda sobre la voz de la marca y el texto se recorta a 2 frases, 1 emoji y 220 caracteres.
- **Corregido el error al reconectar Instagram** ("Invalid Scopes: `pages_read_user_content`"). Ese permiso no lo pide el código: Meta lo arrastra como dependencia de `pages_manage_engagement` dentro del caso de uso de páginas, y estaba sin activar, así que rechazaba el diálogo completo. Activado en el caso de uso (acceso estándar). Verificado abriendo el diálogo con la lista exacta de permisos de la aplicación: ya carga sin error. **Se puede reconectar Facebook e Instagram de cada empresa.**
...
- **Revisión de la aplicación en Meta** (sólo hace falta cuando conecten clientes externos, no para las marcas propias): `pages_manage_engagement` e `instagram_manage_comments` con acceso avanzado; recomendado `pages_read_user_content` (ya activado en estándar); opcionales `pages_manage_posts` e `instagram_content_publish` si se publica desde el CRM. Lista completa en `web/docs/COMENTARIOS_META.md`.
