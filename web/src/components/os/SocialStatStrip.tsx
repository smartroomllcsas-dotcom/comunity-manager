...
import type { SocialGrowth } from '@/lib/os/social';
import type { DmThread } from '@/lib/os/social';
import { InstagramDmInbox } from '@/components/os/InstagramDmInbox';
...
function StatPopout({ metric, onClose }: { metric: 'audience' | 'dms'; onClose: () => void }) {
                ...
                <div className="mb-4 flex flex-wrap gap-1.5">
                  {data.map((s) => {
                    ...
                    return (
                      <button
                        key={s.key}
                        onClick={() => setActive((prev) => { const next = new Set(prev); next.has(s.key) ? next.delete(s.key) : next.add(s.key); return next; })}
                        className={`flex items-center gap-1.5 rounded-sm-t border px-2 py-1 font-mono text-[10px] transition-colors ${on ? 'border-os-border-strong text-os-text' : 'border-os-border text-os-dim hover:text-os-muted'}`}
                      >
...
function MetricTile({ label, headline, headlineClass = '', sub, range, onRange, onOpen }: {
  label: string; headline: string; headlineClass?: string; sub: React.ReactNode;
  range: Range; onRange: (r: Range) => void; onOpen: () => void;
}) {
  return (
    <div
...
function DmInboxPopout({ threads, onClose }: { threads: DmThread[]; onClose: () => void }) {
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => e.key === 'Escape' && onClose();
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-os-bg/70 p-3 backdrop-blur-sm sm:p-6" onClick={onClose}>
      ...
      >
        ...
        <div className="p-5">
          <InstagramDmInbox threads={threads} onClose={onClose} />
        </div>
      </div>
    </div>
  );
...
export function SocialStatStrip({
  audienceTotal, audienceGrowth, totalDms, dmGrowth, platformsCount, dmThreads,
}: {
  ...
  const unreplied = dmThreads.filter((t) => t.unread > 0).length;
  ...
  return (
    <>
        ...
        <MetricTile
          ...
          sub={
            <span className="flex items-center gap-1">
              ...
              {dmPct != null && (dmPct >= 0
                ? <TrendingUp className="h-3 w-3 text-os-ok" />
                : <TrendingDown className="h-3 w-3 text-os-err" />)}
      ...
      {popout === 'inbox' && <DmInboxPopout threads={dmThreads} onClose={() => setPopout(null)} />}
    </>
  );
}
