...
/**
 * Identidad para la Home. Vive en `@/lib/auth/page-identity` porque el mismo
 * problema (dos logins vivos) lo tenían varias páginas más.
 */
export async function resolveHomeIdentity(): Promise<{ userId: string; orgId: string } | null> {
  const { resolvePageIdentity } = await import("@/lib/auth/page-identity");
  return resolvePageIdentity();
}
